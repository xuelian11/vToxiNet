...
class AopDnnDropout(nn.Module):

    def __init__(self, num_frag, num_MIE, num_gene, ptw_direct_gene_map, ptw_size_map, num_hiddens, dG, dG_gene, drop_rate):
        super(AopDnnDropout, self).__init__()
    ...
    def cal_ptw_dim(self, ptw_size_map):
        ...
        :return: dictionary ptw_dim_map that stores the pathway and corresponding output size, should be same
                 for all the pathways, which is the num_hiddens, DrugCell used 6
        """
        ptw_dim_map = {}
    ...
    def forward(self, x):
        ...
        x = self.dropout(layer_data[0]) # dropout input
        mie_out = self._modules['MIE_layer'](x)  # [n_frag, n_MIE]
        
        # element-wise multiplication of the MIE layer output and MIE data, which act as the 'activation' function
        ptw_hidden_out_map['MIE_out'] = self.dropout(torch.mul(mie_out, layer_data[1])) # n_MIE
        ...
        ptw_hidden_out_map['gene_out'] = self.dropout(torch.mul(gene_out, torch.abs(layer_data[2])))
